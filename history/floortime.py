...
enName="B 9"
# areaName="Building 9"
filename="~/Documents/newdata1/"+workerName+".xlsx"
# keyword=["时间", "楼栋", "单元", "楼层", "步数"]
# noon="12:00:00"
# workArea=[["9栋", "1单元"], ["9栋", "2单元"]]#, ["19栋", "2单元"], ["4栋", "1单元"]]
...
metrics=pd.DataFrame(index=sheets, columns=["平均层作业时长",
                                            "平均层作业步数"])
...
for s in sheets:
    ...
    avgTime=int(np.average(df["总作业时间"].values)/60)
    avgStep=int(np.average(df["总工作量"].values))
    metrics.loc[s, "平均层作业时长"]=avgTime
    metrics.loc[s, "平均层作业步数"] = avgStep
    #筛选出楼层的数据点
    #for i in range(rows):
    ...
    # floorList=medFilter(floorList, 5)
    #
...
metrics.to_excel("~/Documents/newdata1/"+enName+".xlsx")